...
import { Sparkles, RefreshCw, FileCode, Play, LoaderPinwheel, Loader2 } from 'lucide-react';
import { SelectDemo } from './SelectDropdown';
import { CODE_SNIPPETS, items } from '@/constant/data';
import Console, { ConsoleResponse } from './Console';
...
function CodeEditor() {
    ...
    const [language, setLanguage] = useState<keyof typeof items>("javascript")
    const [value, setValue] = useState<string >(CODE_SNIPPETS["javascript"])
    const [consoleOutput, setConsoleOutpur] = useState<ConsoleResponse>()
    // Prevent hydration mismatch
    const {mutate:createConsole , isPending ,error , data} = useCreateConsole()
    ...
    const getTeckStack = (stack: keyof typeof items) => {
        setLanguage(stack)
